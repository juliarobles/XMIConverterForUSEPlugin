...
public abstract class AbstractAntlrParser extends AbstractParser {
	...
	public IParseResult doParse(Reader reader) {
		try {
			return parse(getDefaultRuleName(), new ANTLRReaderStream_FIX(reader));
		} catch (IOException e) {
	...
	protected IParseResult parse(String ruleName, CharStream_FIX in) {
		return doParse(ruleName, in, nodeModelBuilder.get(), 0);
	}

	protected IParseResult doParse(String ruleName, CharStream_FIX in, NodeModelBuilder nodeModelBuilder, int initialLookAhead) {
		TokenSource_FIX tokenSource = createLexer(in);
	...
	public IParseResult parse(ParserRule rule, Reader reader) {
		try {
			IParseResult parseResult = parse(rule.getName(), new ANTLRReaderStream_FIX(reader));
			return parseResult;
		} catch (IOException e) {
			throw new WrappedException(e);
		}
	...
	public IParseResult parse(RuleCall ruleCall, Reader reader, int initialLookAhead) {
		try {
			...
			IParseResult parseResult = doParse(ruleCall.getRule().getName(), new ANTLRReaderStream_FIX(reader), builder, initialLookAhead);
			return parseResult;
		} catch (IOException e) {
			throw new WrappedException(e);
		}
	...
	protected TokenSource_FIX createLexer(CharStream_FIX stream) {
		Lexer lexer = lexerProvider.get();
		lexer.setCharStream(stream);
		return lexer;
	}

	protected XtextTokenStream createTokenStream(TokenSource_FIX tokenSource) {
