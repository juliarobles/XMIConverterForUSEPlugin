...
public abstract class AbstractLexerBasedConverter<T> extends AbstractValueConverter<T> implements IValueConverter.RuleSpecific {
	...
	protected void assertTokens(T value, String result) {
		TokenSource_FIX tokenSource = getTokenSource(result);
		assertTokens(value, tokenSource, result);
	}

	protected void assertTokens(T value, TokenSource_FIX tokenSource, String escapedString) {
		if (tokenSource == null)
			return;
		Token_FIX token = tokenSource.nextToken();
	...
	protected ValueConverterException createTokenTypeMismatchException(T value, String escapedString, Token_FIX token) {
		return new ValueConverterException("The value '" + value + "' is an invalid " + getRuleName(), null, null);
	}

	protected ValueConverterException createTokenContentMismatchException(T value, String escapedString, Token_FIX token) {
		return new ValueConverterException("The value '" + value + "' is an invalid " + getRuleName(), null, null);
	}

	protected TokenSource_FIX getTokenSource(String escapedValue) {
		Lexer result = getLexer();
		if (result == null)
			return null;
		result.setCharStream(new ANTLRStringStream_FIX(escapedValue));
		return result;
	}
